...
* To check that the failover has actually happened you can use [`ROLE`](/commands/role), `INFO REPLICATION` (which indicates "role:master" after successful failover), or `CLUSTER NODES` to verify that the state of the cluster has changed sometime after the command was sent.
